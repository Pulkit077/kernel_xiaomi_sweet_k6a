...
void fts_read_osc_trim(void)
{
	int ret = 0;

	FTS_FUNC_ENTER();

	ret = fts_read_reg(FTS_REG_FREQUENCY_VAL1, &fts_data->freq_val1);
	if (ret < 0)
		FTS_ERROR("read frequency reg1 fail, ret=%d", ret);
	ret = fts_read_reg(FTS_REG_FREQUENCY_VAL2, &fts_data->freq_val2);
	if (ret < 0)
...
void fts_write_osc_trim(void)
{
	int ret = 0;

	FTS_FUNC_ENTER();

	ret = fts_write_reg(FTS_REG_FREQUENCY_VAL1, fts_data->freq_val1);
	if (ret < 0)
		FTS_ERROR("write frequency reg1 fail, ret=%d", ret);
	ret = fts_write_reg(FTS_REG_FREQUENCY_VAL2, fts_data->freq_val2);
	if (ret < 0)
